...
one-hot encoded leg index $\mathbf{f_c}^{\prime}$ and the GaitNet input
$\mathbf{x_g}$:
...
Descriptions of the first five terms can be found
in \autoref{subsec:methodology-contactnet-architecture}. The
additional terms, compared to the footstep evaluation network input (\ref{eq:contactnet-x}), are
$\mathbf g$ and $\mathbf c$, being the gravity vector and contact state respectively. Using $\mathbf{x_g}$ and
