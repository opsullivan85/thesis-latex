...
the footstep candidate ($\mathbf f_c$) zeroed out in the input. In the 
GaitNet formulation described previously, we pass the estimated heuristic
cost of a footstep action into the model ($\mathbf f_c^\prime$ in \autoref{fig:diagram-gaitnet-architecture}), that is, the expected cost of taking the action
if it were the only motion the robot was performing. For this ablation study
we replace that value with a zero during the training and evaluation process
to see how it affects the network without changing the shape of the network
at all.
