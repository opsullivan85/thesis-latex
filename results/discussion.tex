...
\hl{This section reads more like a conclusion in some parts. I
  suggest that you remove the Discussion subsection and add its content
  (your discussions) under the relevant subsections. I really think
  that you need to elaborate more on your results. Dividing this
  section and putting them under relevant subsections might help with
this elaboration.}
...
where the baseline's single leg motion strategy fails. The
performance gap narrows only in the easiest conditions (at 0.05~m/s
...
\resGaitNetMeanSwingDuration{} with only modest variation (standard deviation of
...
from \resGaitNetSurvivalRate{} to \resCostAblatedSurvivalRate{}
survival rate. This result challenges the initial assumption that
providing the learned heuristic costs would accelerate training and
improve final performance. Several factors likely contribute to this
outcome. The correlation analysis
...
improved low level control. The limited impact of dynamic swing
duration suggests that either the feature requires more sophisticated
use cases to demonstrate value, or the current implementation does
not adequately expose its potential benefits. Future work
incorporating prediction horizons or more complex terrain may better
